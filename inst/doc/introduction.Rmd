...
```
\begin{itemize}
\item The histogram on the right shows the values for Solar.R.
\item The selected value of Solar.R is 207. By clicking on the histogram,
you can change this selection.
\item The main display (on the left) shows the predicted values of Ozone, varying Wind
and fixing Solar.R at $f_2 =207$.
\item Only observations which have Solar.R values near 207 are shown,i.e.,
observations whose $d_i = |x_{i,2} - f_2|<1$, where Solar.R values  are measured
in standard units.
\item The nearness required can be increased or decreased from 1 by moving the Similarity Threshold
slider. 
\item Also, point colours
...
\item You can exchange the roles of Solar.R and Wind by choosing Solar.R in the `Choose a sectionvar` menu.
\end{itemize}
...
\begin{itemize}
\item The plot on the right shows the values for Solar.R and Temp. By clicking
on this plot, you can change the selected values.
\item The main display (on the left) shows the predicted values of Ozone, varying Wind
and fixing Solar.R and Temp.
\item Only observations which have Solar.R and Temp values near the pink
...
\item Again, point colours
fade to reflect the distances $d_i$.
\item The plot on the right shows all the observations, but points near the pink cross
...
Condvis2 does not directly support confidence intervals, but they are fairly easy to display.
The loess fit offers standard errors, which we use for confidence bounds:
...
\begin{itemize}
\item The simplest method chooses random observations in conditioning space
to form the sections.
\item Click on the arrow below `Tour Step` to watch.
\item You can increase the number of random points via the `Tour Length` slider.
\item If you want a smoother appearance to the tour, increase `Interp steps` from 0.
\end{itemize}
